...
pos=cglayout([1,1] , OXMargin=[11,11], OYMargin=[10,10], XGap=10, YGap=10  )
...
ax[0]=ptr_new(xa)
ax[1]=ptr_new( findgen(ny)/ny*2-1 )
...
fname="contphase"+string(nfile, format='(I06)')
...
imax=max(dat)
;cgimage, r, pos=px
cgcontour, r,*ax[0], *ax[1], /noerase,  pos=px,  $ 
    xrange=[-50,150],$
    yrange=[-1.5,3.5],$
    charsize=charsize,$
    /fill, $
...
cgcontour, r, *ax[2], *ax[3] , /noerase,  pos=px, $
    xrange=[-50,150],$
    charsize=charsize,$
    /fill, $
    nlev=64,$
...
;device,/close
cgps_close, /jpeg,  height=ys, /nomessage, delete_ps=0
