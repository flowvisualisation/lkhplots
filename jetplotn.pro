;pro jetplotn, var=var, field=field, n=nfile, mfield=mfield, current=current
...
r=h5_read(nfile, /rho, /B, /v,/p)
grd_ctl,model=nfile, grid
tnirv=nfile

rho=transpose(reform(r.rho))
prs=transpose(reform(r.p))
bx1=transpose(reform(r.b(0,*,*)))
bx2=transpose(reform(r.b(1,*,*)))
bx3=transpose(reform(r.b(2,*,*)))
vx1=transpose(reform(r.v(0,*,*)))
vx2=transpose(reform(r.v(1,*,*)))
vx3=transpose(reform(r.v(2,*,*)))
x1=grid.x
x2=grid.y
nx1=size(x1, /dim)
nx2=size(x2, /dim)
...
theta2=congrid(theta2, nx1,nx2)
rr=congrid(rr, nx1,nx2)
...
cgcontour, data, xp1, xp2, pos=pos,  /nodata,/noerase, color='black', $
   title='log!D10!N '+rhotext+','+' t='+string(tnirv, format='(F4.1)'),$
   xtitle='R',$
   ytitle='Z'
imin=min(data)
imax=max(data)
...
yy=x2
ma2=ma
polar,ma2,xx,yy,sample=10
xx=x1
yy=x2
mf2=mf
polar,mf2,xx,yy,sample=10

	 cgcontour, ma2, xp1,yp1, levels=1, color='red', /overplot, c_thick=2.0, label=0
	 cgcontour, mf2, xp1,yp1, levels=1, color='yellow', /overplot, c_thick=2.0, label=0
