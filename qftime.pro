...
ssz=strlen(dirname)
dirtag=strmid(dirname,18,ssz-1-18) & print, dirtag
...
fname="${HOME}/Documents/results/"+dirname+"/"+"qftime"+dirname+ttag
...
pos=cglayout([1,1] , OXMargin=[11,2], OYMargin=[9,6], XGap=4, YGap=11)
...
tstr(3)=dirtag+', t='+timetag+',Q!Df!N'
tstr(4)='k |B(k)|!U2!N'
...
cgplot,karr[2:nx1*1.-1], (qcfrac[2:nx1*1.-1]), pos=pos[*,0], /noerase, /xlog, xrange=[0.2,12], $
    yrange=[.2,1],$
    color='black', ytitle='Q!Df!N', title=tstr(3), xtitle='kH/2'+pistr , charsize=charsize
cgplot,karr[2:nx1*1.-1], (qdfrac[2:nx1*1.-1]),  /overplot, color='blue'
items=['disk','corona']
al_legend, items, linestyle=[0,0],color=['blue','black'], $
   ; /right, $
    charsize=charsize,linsize=0.15


if ( usingps ) then begin
;device,/close
cgps_close, DELETE_PS=0, /jpeg,  Width=1900, /nomessage
;set_plot,'x'
endif else begin
;set_plot,'x'
fname2=fname
