pro cgexp, vx1,vx2, rho, prs, t , nlast, nx1,nx2,x1,x2, background,dx1,dx2, zbuf=zbuf
growth=0
cg=0
hires=0
pload,0,/silent
...
expv1=initv1*exp( 0.001*t[nfile])
...
grtitle='(Total |V!DX,Z!N|/(V!DX!N!U2!N +  V!DZ!N!U2!N)!U1/2!N' 
lgrtitle='Log'+grtitle

cgplot, tnorm, (tvz2), title=lgrtitle ,   xrange=[0.1,12],yrange=[0.1,1e1], xtitle='t / t!Dsound crossing!N', ytitle=lgrtitle,    Color='black',linestyle=0, /ylog
cgplot, tnorm, (tvx2), /overplot,   Color='red',linestyle=2
al_legend, ['V!DZ!N!U2!N','V!DX!N!U2!N'], $
      LineStyle=[0,2], Color=['black','red'], charsize=legchar, /left


  cgText, 0.5, 0.95, ALIGNMENT=0.5,$
   'KHI PLUTO, t='+string(t(nfile),format='(F4.1)')+ $
      ', aspect ratio '+string(max(x2)/max(x1), format='(F5.2)')+$
     ', growth rate'+string(growth, format='(F7.2)')+$
      ' ',$
   /normal, charsize=xychar, color='black'
