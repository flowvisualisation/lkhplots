...
qarr=[0.5,0.7,0.9,1.1,1.3,1.5,1.7,1.9]
tptr=ptrarr(12)
bptr=ptrarr(12)
rptr=ptrarr(12)
aptr=ptrarr(12)
...
thick=2.
qsm=1
cgplot, *tptr[0]/2./!DPI, smooth(*bptr[0],qsm), /ylog, yrange=[ymin,ymax], color=cl[0], xrange=[0,tmax], lines=ls[0], $
    charsize=charsize,$
    xtitle='t, orbits', $
    ytitle='BxBy '

for i=1,7 do begin
cgplot, *tptr[i]/2./!DPI, smooth(*bptr[i], qsm), /overplot, color=cl[i], lines=ls[i]


endfor

for i=0,7 do begin
items[i]='q='+string(qarr[i], format='(F3.1)')+ ' !9a!X='+string(meanalpha, format='(F7.4)')
dat=*bptr[i]
sz=size(dat, /dimensions)
meanalpha=mean(dat[sz(0)*.25:sz(0)-1]  )
aptr[i]=ptr_new(meanalpha)
print, qarr[i], meanalpha
endfor
