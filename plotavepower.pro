...
fname='q'+string(q*10,format='(I02)')+"powerave"+vtag
...
lines[3]=3
 ...
 cgplot,k1, k1*maxave, /xlog, /ylog, xrange=[1,100], yrange=[1e-12,1e-2], xstyle=1, color=colors['red'], ytitle='q='+string(q, format='(F4.1)')+' , k |DFT('+vtag2+')|!U2!N', xtitle='kL', pos=[0.15,0.15,0.95,0.95], charsize=0.5*cgdefcharsize()
...
al_legend, items[0:3], colors=colors[0:3], linestyle=lines[0:3], charsize=0.5*cgdefcharsize(),  $   
    pos=[20,1e-2],$
   ; /right, $
    linsize=0.25, thick=thick

r= histogram( angle[0:na-1], locations=xbin)  
cgplot, xbin, r, xtitle='Angle !Uo!N', ytitle='Frequency', /noerase, pos=[0.25,0.28,0.45,0.5], charsize=0.5*cgdefcharsize(), xticks=3,xtickinterval=10;, xrange=[20,50]
