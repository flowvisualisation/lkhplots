...
r=h5_read(0 ,/v,/b, dir=dirtag)
grd_ctl,model=0, g,c, dir=dirtag 
vy3d0=reform(transpose(r.b(1,*,*,*)))
vy0=transpose(r.b(1,0,*,*))
getlast, nfile, dir=dirtag
get2last, nfile2, dir=dirtag
;nlast=40000
;nfile=closestshear[i]
;PLOAD, nfile, DIR = dirtag
r=h5_read(nfile ,/v,/b, dir=dirtag, /remap)
grd_ctl,model=nfile, g,c, dir=dirtag
vx3d=reform(transpose(r.b(0,*,*,*)))
vy3d=reform(transpose(r.b(1,*,*,*)))-vy3d0
vz3d=reform(transpose(r.b(2,*,*,*)))
...
r=h5_read(nfile2 ,/v,/b, dir=dirtag, /remap)
grd_ctl,model=nfile2, g,c, dir=dirtag
vx3d=reform(transpose(r.b(0,*,*,*)))
vy3d=reform(transpose(r.b(1,*,*,*)))
vz3d=reform(transpose(r.b(2,*,*,*)))
