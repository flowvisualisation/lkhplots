...
pos= cglayout([1,2] , OXMargin=[9,1], OYMargin=[9,6], XGap=7, YGap=12)
...
fname=tag+'lines';+string(nfile,format='(I08)')
...
ymax=max(k*f0unnorm)
cgplot, k,k*f0unnorm,pos=pos[*,0], /noerase, xtitle='k', /xlog,/ylog, $
    xrange=[1,max(k)], $
    yrange=[1e-3*ymax,10*ymax], $
    charsize=charsize, $
color='red',$
    title=qstr+' , log!D10!N|k B!U2!N(k!Dx!N,k!Dy!N)|' ;,$
...
cgplot, k,k*f0,pos=pos[*,1], /noerase, xtitle='k', /xlog,/ylog, xrange=[1,max(k)], charsize=charsize,$
color='red',$
...
ymin=1e-3*ymax
;cgplot, tnorm, bx2s , pos=[0.1,0.1,0.98,0.2], /noerase, /ylog, yrange=[ymin,ymax], color=['red'], ytitle='B!Dx,y,z!N', xtitle='time/2'+pi_str+'  (orbits)'+' , '+dirtag, charsize=charsize
