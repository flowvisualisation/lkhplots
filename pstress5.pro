qsm=3
   cgDisplay, WID=1,xs=1500, ys=1000, xpos=200, ypos=700
   mychar=1.1
...
zsum=findgen(mynum)
zsum(*)=0.
maxsum=findgen(mynum)
maxsum(*)=0.
minsum=findgen(mynum)
minsum(*)=0.
zave=findgen(mynum)
zave(*)=0.
maxave=findgen(mynum)
maxave(*)=0.
minave=findgen(mynum)
minave(*)=0.
pave=findgen(mynum)
pave(*)=0.
ave=0
...
nstep=2
   ...
   posx = cglayout([3,2] , OXMargin=[6,1], OYMargin=[4,1], XGap=6, YGap=4)
   !p.multi=[0,1,4]
   kx=findgen(181)+1
   b=findgen(100)/100*1e-2+1e-12

j=0
!p.charsize=2.9
q=getq(0)
   cgplot, kx,b,/xlog, /ylog,/nodata, pos=posx[*,j], /noerase, xstyle=1, ytitle=string(q)
   p1 = !P & px1 = !X & py1 = !Y
j=1
...
fslzoom=fsl;[nx2*zmn:nx2-1, nx3*zmn:nx3-1]
fsl2zoom=fsl2;[nx1*zmn:nx1-1, nx3*zmn:nx3*zmx]
