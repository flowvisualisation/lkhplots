...
for i=0,dsx-1 do begin
    ...
    ;cut45[i,nfile]=shftft[nx/2+i,ny/2,nz/2]
    ;cut135[i,nfile]=shftft[nx/2,ny/2+i,nz/2]
endfor
...
ytick=1
ytickf=''
    ytit='log!D10!N k|FFT('+vtag+')|!U2!N'
endif
k1h=k1/sqrt(1.)
cgplot,k1h,k1h* cut45[*,nfile], $
    /xlog, $
    /ylog, xrange=[1,100], color=colors[0], linestyle=lines[0], xtitle='k L', $
    ytitle=ytit,$
    ytickformat=ytickf,$
    ystyle=1,$
    yrange=[1e-7,10],$
...
if ( nfile eq 0 ) then begin
 ...
 cgtext, 2,1e-6, 't='+string(plotlist[nfile]-6,format='(I2)')+' !9W!X!U-1!N'
 endfor
