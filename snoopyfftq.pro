...
dataptr[ 1]=ptr_new(smooth(shift(abs(fft(final)),nx1/2,nx2/2),1))
...
endelse
   ...
   cgLoadCT, 33
   pos = cglayout([2,1] , OXMargin=[4,12], OYMargin=[5,9], XGap=12, YGap=2)
   FOR j=0,1 DO BEGIN
     p = pos[*,j]
     d= *dataptr(j)
  ...
  pos=p, title=titlestr(j), Charsize=cgDefCharsize()*0.5
   ...
   ENDFOR
   cgText, 0.5, 0.92, /Normal,  'vz and DFT(vz), t='+string(mytime), Alignment=0.5, Charsize=cgDefCharsize()*0.5
