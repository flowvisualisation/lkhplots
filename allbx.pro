...
nplots=10
 pos=cglayout([1,nplots] , OXMargin=[4,1], OYMargin=[7,1], xgap=0, YGap=0)
...
print, dirtag
PLOAD, 0, DIR = dirtag, var='vx2', /nodata
;vy0=vx2(*,*,0)
nfile=nlast
...
;print, 'q=',qarr[i],' , a/b', aa[2]/aa[3]
 ;   cgcontour, gft, kx,ky,  pos=px, color='Black', label=0, /overplot ;, lev=[1e-5,1e-4, 1e-3]
    cgtext, -2.5,0.2 , 'q='+string(qarr[i],format='(F4.1)') ,color='black', charsize=charsize
endfor
