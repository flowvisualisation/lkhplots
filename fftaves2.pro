...
xfftcut[i, loadnum]=alog10(qxdd(i+1,i,0))
;print, loadnum,xfftcut(i,loadnum)
endfor

for i=0,ny/2-1 do begin
yfftcut[i, loadnum]=alog10(qxdd(i+1,ny-i-1,0))
endfor

zfftcut[0:nz/2-1, loadnum]=alog10(qxdd(1,1,1:nz/2))
...
cgplot, x, smooth(xfftcut(*,nplot),qsm),   $
    xrange=[0.9,1e2], $
    /xlog,$
    pos=pos1[*,nplot],$
    /noerase,$
    charsize=mychar,$
     ...
     yrange=[-9,1], color=colors[0], $
    xtitle='log!D10!N|k|',$
    ytickf=ytickf,$
    ytitle=ytit


cgplot, x,smooth(yfftcut(*,nplot),qsm), /overplot, color=colors[1], linestyle=lines[1]


cgplot, x,smooth(zfftcut(*,nplot),qsm), /overplot, color=colors[2];, linestyle=lines[2]
