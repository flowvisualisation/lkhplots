...
titlestr[ 0,*]='DFT '+vtag+'(k!D1!N,k!D3!N)'
titlestr[ 1,*]='DFT '+vtag+'(k!D2!N,k!D3!N)'
...
xq= findgen(nyq/2)*sin(!DPI/2-angle)+nyq/2
yq=-findgen(nyq/2)*cos(!DPI/2-angle)+nyq/2
k1min=sqrt(  (  findgen(nxq/2)*cos(angle)  )^2+( findgen(nyq/2)*sin(angle) )^2)
   pmaxstress=interpolate (dat ,xq,yq)
   endif
    ...
    ttag='t='+string(nfile,format='(I3)')+', q='+string(q,format='(F4.1)') ;+' orbits'
...
for j=-nx/2,ny/2-1 do begin
for k=-nx/2,ny/2-1 do begin


rad=sqrt((1.d*i)^2 +(1.d*j)^2 +(1.d*k)^2)

radint=round(rad)

powave[radint] += square[nx/2+i,ny/2+j,ny/2+k]/rad^2/!DPI/4
...
cgplot,k1h, k1h*maxstress, /xlog, /ylog, xrange=[1,128],yrange=[yrmin,yrmax], $
;ystyle=1, $
color=colors[0], $
