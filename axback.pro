...
titlestr[8,*]='bx^2+by^2'
   ...
   pos = cglayout([1,1] , OXMargin=[5,5], OYMargin=[5,7], XGap=3, YGap=7)
   ;FOR j=0,8 DO BEGIN
   j=0
     p = pos[*,j]
     j=7
  ...
  cgcontour,xx#yy, xx,yy , /nodata, /noerase, xtitle='x', pos=p, title=titlestr(j), Charsize=cgDefCharsize()*0.5
   ...
   ;ENDFOR
   cgText, 0.5, 0.96, /Normal,  'SDI time='+string(time/2./!DPI, format='(F6.2)'), Alignment=0.5, Charsize=cgDefCharsize()*1.25
...
cgcontour,bet, xx,yy,pos=p, levels=1, /overplot
