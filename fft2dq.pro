...
print, nfile
;bx1=transpose(reform(d.b(0,*,*,*)))
;bx2=transpose(reform(d.b(1,*,*,*)))
;bx3=transpose(reform(d.b(2,*,*,*)))
...
pos= cglayout([2,2] , OXMargin=[11,1], OYMargin=[31,6], XGap=11, YGap=19)
...
levh1=total (sqrt(bx1^2+bx2^2+bx3^2),3)
levh0=total (sqrt(bx1^2+bx2^2+bx3^2),3)

levh1=remapfunc ( levh1 , t[nfile], nx1,nx2, q,  x1,x2,dx2)
levh0=remapfunc ( levh0 , t[nfile], nx1,nx2, q,  x1,x2,dx2)
...
omega=1.0
Ly=2.0
dt=nirvtime mod  0.666666666666666666666666666666666666d
...
x2d=rebin(reform(x1,nx1,1),nx,ny)
jimag=complex(0,1)

cfft1=fft(levh1,dimension=2)
cfft1shift=cfft1*exp ( -jimag * ky2d * x2d *2 *!PI *qomegat_Ly ) 
cfft2=fft(cfft1shift, dimension=1 )
fftg1=shift(cfft2, nx1/2, nx2/2)


cfft1=fft(levh0,dimension=2)
cfft1shift=cfft1*exp ( -jimag * ky2d * x2d *2 *!PI *qomegat_Ly ) 
cfft2=fft(cfft1shift, dimension=1 )
fftg0=shift(cfft2, nx1/2, nx2/2)



fh1=(nx1*nx2*abs( fftg1))
...
lg10bn='log!D10!N|B!Dnorm!N!U2!N|'
titstr(0)=qstr+' , ' +lg10bn
titstr(1)=lg10bn 
titstr(5)='dummy'
titstr(3)=lg10bn
titstr(4)=lg10bn
...
cgplot, k,k*f0unnorm,pos=pos[*,2], /noerase, xtitle='k', /xlog,/ylog, $
    ...
    charsize=charsize, $
color='red',$
    title='log!D10!N|k B!U2!N(k!Dx!N,k!Dy!N)|' ;,$
...
cgplot, k,k*f0,pos=pos[*,3], /noerase, xtitle='k', /xlog,/ylog, xrange=[1,max(k)], charsize=charsize,$
color='red',$
    title='log!D10!N|k B!Dnorm!N!U2!N(k!Dx!N,k!Dy!N)|' ;,$
;    ytitle='log |B!Dnorm!N!U2!N(k!Dx!N,k!Dy!N)|'
...
readcol,'averages.dat',n,tn1,b2,bxby,ruxuy
...
ymax=max( [ b2,bxby, ruxuy] )
ymin=1e-3*ymax
mpx=pos[0,0]
cgplot, tnorm, b2 , pos=[mpx,0.1,0.98,0.2], /noerase, /ylog, yrange=[ymin,ymax], color=['red'], ytitle='B!Dx,y,z!N', xtitle='time/2'+pi_str+'  (orbits)', charsize=charsize
cgplot, tnorm, abs(bxby) , /overplot, color=['blue']
cgplot, tnorm, abs(ruxuy) , /overplot, color=['green']
