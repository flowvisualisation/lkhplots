...
cgloadct,33
items=['MRI','MRI','Kato+ (1995)']
lines=[0,0,0]
colors=['red','red', 'green']

cgplot, q,alpha, xtitle='Shear rate, q= -dln!9W!X/dlnr', ytitle='Normalized Turbulent Stress, T!Dr!9f!X!N/P', psym=psym[0], linestyle=0, xrange=[0,2], color=colors[0], yrange=[-1,7], pos=[0.1,0.2,.95,.95]

;cgplot, q, alpha, /overplot, psym=psym[1], color=colors[1]
;cgplot, q,alpha,   linestyle=0, /overplot
qexp=4.5
mystr=string(qexp, format='(F4.1)')
;cgplot, q, (q^8)/(1.5^8),     linestyle=lines[0],color=colors[2], /overplot, psym=psym[2]
;cgplot, q, (q^qexp)/(1.5^qexp), linestyle=lines[1],color=colors[1], /overplot
nitems=0
al_legend,  items[0:0], lines=lines[0:0], color=colors[0:0], linsize=0.05, psym=psym[0:0]
